...
export function ExpenseAttachments({ expenseId, initialFiles }: ExpenseAttachmentsProps) {
  ...
  const folder = `agencyos/expenses/${expenseId}`;
  ...
  const uploadOne = async (file: globalThis.File) => {
    ...
    formData.set("scope", "expense-attachment");
        ...
        <CardContent className="space-y-4">
          ...
          {files.length === 0 && !uploading ? (
            <p className="text-muted-foreground text-sm">
              No attachments yet. Upload PDFs, receipts, or images linked to this expense.
            </p>
          ) : (
                        ...
                        <div className="bg-card overflow-hidden rounded-xl border shadow-sm transition-shadow hover:shadow-md">
                          ...
                          >
                            <div className="bg-muted/40 flex aspect-4/3 items-center justify-center rounded-lg border border-dashed">
                              <FileText className="text-primary h-14 w-14 opacity-80" aria-hidden />
                            </div>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Remove {deleteTarget?.name} from this expense? The file will be deleted from storage and cannot be
              undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
