...
function SidebarProvider({
  ...
  // Close mobile sidebar when route changes (e.g. after selecting a page)
  React.useEffect(() => {
    setOpenMobile(false)
  }, [pathname])
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          data-slot="sidebar"
          data-mobile="true"
          className="bg-sidebar text-sidebar-foreground w-[280px] max-w-[95vw] p-0 [&>button]:hidden"
          style={
            {
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  const { toggleSidebar } = useSidebar()
  const t = useTranslations("common")
  const toggleLabel = t("sidebarToggle")
...
function SidebarMenuSkeleton({
  ...
  const width = React.useMemo(() => {
    ...
    for (let i = 0; i < stableId.length; i++) {
      h = (h + stableId.charCodeAt(i) * (i + 1)) % 40;
    }
    return `${50 + h}%`;
  }, [stableId]);
